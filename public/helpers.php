...
function save_tag_cache($name=null,$type=null){
    ...
    $tmp_cache = 'tag_all';
    Cache::forever($tmp_cache, $all_tags);
    if ($name!=null){
        unset($all_tags);
        $all_tags[]=$name;
    }
...
function  save_cate_cache($category_id){
    ...
    $enl_categories  =array_keys( Cache::get('enl_categories')->toArray());
    $dark_categories  = array_keys( Cache::get('dark_categories')->toArray());
    $getez_categories  =array_keys( Cache::get('getez_categories')->toArray());
    $avbody_categories  = array_keys( Cache::get('avbody_categories')->toArray());
    ...
    $all=[$enl_all_loop,$dark_all_loop,$getez_all_loop,$avbody_all_loop,$expert_all_loop];
    foreach ($tmp_loop as $k=>$v) {
        if (Cache::has($v)) {
            $$v = Cache::get($v);
        }
    }
    $cate_articles=App\Article::join('articles_map', 'articles.id', '=', 'articles_map.articles_id')->publish()->where('category_id', $category_id)
        ->orderBy('publish_at')
    ...
    foreach ($all as $i=>$tmp_all_loop ) {
        ...
        $all_articles = App\Article::join('articles_map', 'articles.id', '=', 'articles_map.articles_id')->publish()
...
function save_article_cache($id)
{
    ...
    //dd($tmp_cache);
    if (Cache::has($tmp_cache)) {
        Cache::forget($tmp_cache);
    }
    if (Cache::has($tmp_cache_id)) {
        Cache::forget($tmp_cache_id);
    }
    ...
    if ($my_article->tags->pluck('name')->count()>0){
        foreach ($my_article->tags->pluck('name')->all() as $key =>$tag_name){
            save_tag_cache($tag_name);
        }
    }
    Cache::forever($tmp_cache,  $my_article);
    Cache::forever($tmp_cache_id,  $my_article);
...
//dd($id);
    ...
    //if (Cache::has($tmp_cache)) {
    //   $articles = Cache::get($tmp_cache);
    // } else {
    /*** 讀取用 slave*/
    $av_user_id=Auth::av_user()->get()->id;
    $collect=App\AvUserCollect::on('slave')->where('av_user_id',$av_user_id)->lists('created_at','article_id');
    ...
    foreach ($collect as  $k=>$date){
        if (Cache::has('article_' . $k)) {
            $articles->articles[$k]= Cache::get('article_' . $k);
            $i++;
        }
    }
    ...
    //  Cache::forever($tmp_cache, $articles);
    // }
...
function article_instant_content($content)
{
    ...
    $content = preg_replace('/<span(.*?)style="color: #ff0000;(.*?)>(.*?)<\/span>/i', '<adide>$3</adide>', $content);

   // dd($content);
    $content  = preg_replace('/<h[1-6]>(.*?)<\/h[1-6]>/', '<p style="font-weight:bold">$1</p>',  $content );
...
//dd($content);
    ...
    $content = str_replace('../../../', 'http://cdn.eznewlife.com/', $content);
    $content = str_replace('../../', 'http:/cdn.eznewlife.com/', $content);

    $content = str_replace('src="/uploads/', 'src="http://cdn.eznewlife.com/uploads/', $content);
    $content = str_replace('src="/comic/images', 'src="http://cdn.eznewlife.com/comic/images', $content);
    /**暫時的，把all enl拿掉**/
    $content = enl_to_getez($content);
    //移除圖片的 link
...
//   $content= preg_replace('#(<a [^>]+)#', '<p>'.'$1'.'></p',$content);
    ...
    // dd($content);
    //圖片加figure
    $content = strip_tags($content, '<img><p><a><ul><al><li><strong><iframe><aside>');

    $content = preg_replace('/<iframe.*src=\"(.*)\".*><\/iframe>/isU', '<figure class="op-social"> <iframe src="$1" ></iframe></figure>', $content);
    ...
    $content = preg_replace(array('{<p[^>]*><p>}', '{</p></p>}'), array('<p>', '</p>'), $content);
    ...
    //  $content=article_instant_ad($content,$my_ad);
    ...
    $content = preg_replace('/(<p[^>]*>\s?)<strong>(.*?)<\/strong>(<\/p>)/i', '<aside>$2</aside>', $content);
    $content = preg_replace('/(<p[^>]*>\s?)(.*?)(<figure.*?)(<\/p>)/i', '<p>$2</p>$3', $content);
    /*$content = preg_replace('/<a href=".*?(<figure.*?>)<\/a>/', '$1', $content);*/
    $content = str_replace('admin.eznewlife.com', 'cdn.eznewlife.com', $content);
...
function enl_to_getez($content){
    $content = str_replace('admin.eznewlife.com', 'getez.info', $content);
    ...
    foreach ($tags as $tag) {
        ...
        if (strpos($tag->getAttribute('src'),'gif') ){
            ...
            if($size[0] <320){
                ...
                $regex = '<img src="'.$tag->getAttribute('src').'" />';
                // echo $regex;
...
function add_article_attribute($id,$target='id',$do='save')
{
    ...
    foreach($images[0] as $i=> $img){
     // echo $img."<br>";
        $regex = '~<img.*?src="([^"]+).*?\/>~';
      if (!preg_match("/\b(?:(?:http?):\/\/|www\.)[-a-z0-9+&@#\/%?=~_|!:,.;]*[-a-z0-9+&@#\/%=~_|]/i", $img )) {
...
function content_paging($article){
    ...
    $cp = make_content_paging($article->content);
...
function make_content_paging($content){
    $cp = new __content_paging($content);
    return $cp;
}
