                                    ...
                                    <td>
                                        <a href="{{route('articles.show', ['id'=>$collect->ez_map[0]->unique_id,'title'=>hyphenize($title->title)])}}">
                                            @if (File::exists( public_path() . '/focus_photos'."/".$collect->photo) and !empty($collect->photo))
                                                <img src="{!!("/focus_photos/100/".$collect->photo) !!}?lastmod={!!$collect->updated_at!!}" class="img-collect img-thumbnail"  alt="{{$collect->title}}">
                                            @else
                                                <img src="{!!("/images/nophoto.png") !!}" class="img-collect img-thumbnail"  alt="{{$collect->title}}">
                                        @endif
                                    </td>
                                    <td ><a
