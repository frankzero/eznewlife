...
@section('content')
{!!seo_breadcrumb_list('EzNewLife',$categories[$cate_articles[0]->category_id],route('articles.category',['id'=>$cate_articles[0]->category_id,'title'=>hyphenize($categories[$cate_articles[0]->category_id])],false))!!}
    ...
    <div class="col-md-8 blog-main">
        <div class="row center-block article-category-main  hover11 column" >
            @foreach ($cate_articles as $k=> $article)
                ...
                <div class="row center-block article-category-row" onclick="window.location='{{route('articles.show', ['id'=>$article->ez_map[0]->unique_id,'title'=>hyphenize($article->title)])}}'" itemscope itemtype="http://schema.org/ImageObject">
                                ...
                                <div class="row_tag">
                                    ...
                                    <div class="col-xs-11 tag_hidden" >
                                        ...
                                        @foreach ($article->tags->pluck('name')->all() as $key =>$tag_name)
                                            <a href="{{route('articles.tag',['name'=>$tag_name],false)}}" class="btn btn-xs btn-default " rel="tag">{!!$tag_name!!}</a>
                                        @endforeach
                                    </div>
                                </div>
                            @endif
        ...
        <div class="row text-center">
            <div class="col-lg-offset-1">
                {!!show_page_bar_scroll($cate_articles,'btn-success btn-lg bg-enl')!!}
