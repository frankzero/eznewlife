            ...
            <div class="ad-md">
                ...
                <div class="row ">
                    <div class="col-lg-4 col-md-3 col-sm-4 col-xs-6 col-centered"><a
                                class="btn btn-flat btn-danger disabled btn-block"
                                id="seemore" onclick="seemore();"
                                style="background-color: rgb(231, 76, 60);margin-bottom: 20px">點我看文章</a></div>
                </div>


                @endif
                <div class="article-content" @if ($mobile==true)style="display: none" @endif>
                    {!!article_handle_content($article->content_current)!!}
                </div>
                <!--- 文章內頁-->
                @if ($article->content_count >1)
                    <nav class="text-center">
                        <ul class="pagination">
                            @for ($i = 1; $i <= $article->content_count; $i++)
                                <li class="@if($article->content_page==$i  )active @endif"><a href="?page={{$i}}">{{$i}}</a></li>
                           @endfor
                        </ul>
                    </nav>
                @endif
                ...
                <script>
                   ...
                   // $('#ad_block_12').modal('show')
                    ...
                    function gogo(ffb){
                        ...
                        var f1r='<?=fconfig('f1_rate');?>'-0;
                        var f1p='<?=fconfig('f1_pixel');?>'-0;
                            
                        var f2r='<?=fconfig('f2_rate');?>'-0;
                        var f2p='<?=fconfig('f2_pixel');?>'-0;

                        var f3r='<?=fconfig('f3_rate');?>'-0;
                        var f3p='<?=fconfig('f3_pixel');?>'-0;

                        var go = new __gogo(ffb, rffb);
