    ...
    <div id="myCarousel" class="carousel slide center-block" data-ride="carousel">
        ...
        <div class="carousel-inner " role="listbox">
            @foreach ($articles as $k=> $article)
                <div class="item @if ($k==0) active @endif"  onclick="window.top.location.href='{{route('articles.show', ['id'=>$article->ez_map[0]->unique_id,'title'=>hyphenize($article->title)])}}'"><a href="{{route('articles.show', ['id'=>$article->ez_map[0]->unique_id,'title'=>hyphenize($article->title)])}}" target="_top" >
                    @if (File::exists( public_path() . '/focus_photos'."/".$article->photo) and !empty($article->photo))
                        <img src="{!!("/focus_photos/".$article->photo) !!}?lastmod={!!$article->updated_at!!}"
                    ...
                    <div class="carousel-caption">
                        <h3>
                            <a href="{{route('articles.show', ['id'=>$article->ez_map[0]->unique_id,'title'=>hyphenize($article->title)])}}" target="_top">
